                ...
                <table class="table table-striped" v-else>
                  ...
                  <tbody>
                    ...
                    <tr v-for="unit in paginatedUnits" :key="unit.id">
                      ...
                      <td>{{unit.code}}</td>
                      <td>
                        <router-link :to="{name: 'editUnit', params: {id: unit.id}}" class="btn btn-sm btn-info">Edit</router-link>
                        <a @click="deleteUnit(unit.id)" class="btn btn-sm btn-danger">
                           Delete
                        </a>
                      </td>
                    </tr>
...
<div class="modal fade" id="unitModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <form @submit.prevent="createUnit" id="unitForm">
    ...
    methods:{
        ...
        loadUnit(){
          ...
          axios.get('/api/unit')
            .then(res=>{
               this.units = res.data;
               this.pagination.totalPage = this.units.length / this.pagination.rowPerPage
               this.paginatedUnits = this.units.slice(0, this.pagination.rowPerPage);
        ...
        clickCallback: function(page) {
          ...
          this.paginatedUnits = this.units.slice(skip, skip+this.pagination.rowPerPage);
        },
